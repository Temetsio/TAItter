...
?>
<!doctype html><html><head><meta charset="utf-8"><title><?=htmlspecialchars($row['username'])?></title></head><body>
<div style="max-width:800px;margin:20px auto;">
  ...
  while ($p = $r->fetch_assoc()) {
    ...
    $c = preg_replace('/#([A-Za-z0-9_åäöÅÄÖ\-]+)/u','<a href="index.php?hashtag=$1">#$1</a>',$c);
    $c = preg_replace('/@([A-Za-z0-9_]+)/','<a href="profile.php?u=$1">@$1</a>',$c);
    echo "<div class='card'><div><small>{$p['created_at']}</small></div><div>$c</div></div>";
  }
  ?>
</div>
</body>
